...
// Be sure to set numops to 0 before calling this the first time.

#define MAX_BATCH_OPS (200)

#define MAKE_RD_BATCH_ENTRY(batch_ptr,batch_idx,cpu_idx,msr_address,data)		\
do{												\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].cpu	= cpu_idx;		\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].isrdmsr	= 1;			\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].msr	= msr_address;		\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].msrdata	= data;			\
	batch_ptr[batch_idx].numops++;								\
}while(0)

#define MAKE_WR_BATCH_ENTRY(batch_ptr,batch_idx,cpu_idx,msr_address,data)		\
do{												\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].cpu	= cpu_idx;		\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].isrdmsr	= 0;			\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].msr	= msr_address;		\
	batch_ptr[batch_idx].ops[ batch_ptr[batch_idx].numops ].msrdata	= data;			\
	batch_ptr[batch_idx].numops++;								\
}while(0)

#endif	//MSR_SAFE_HEADER_INCLUDE
