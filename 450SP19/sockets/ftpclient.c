...
#define SERVER_PORT_ID 8090
#define CLIENT_PORT_ID 5086
...
int main(int argc,char *argv[])
{
    ...
    char in_buf[MAXSIZE];
    if(argc != 3) {
        printf("error: usage : %s filename IP-dotted-notation\n", argv[0]); 
        exit(0);
    }
    no_writen = 0;
    num_blks = 0;
    ...
    */ 
    ...
    /* Now the client is reading information from the server regarding
       how many full blocks of size maxsize it can expect. It also
       receives the number of bytes remaining in the last partially
       filled block, if any */  
