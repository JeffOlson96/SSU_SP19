...
+		03/1/19																   +
+		Bugs:															       +
+			tr with quotes does not work									   +
+			input files need a file extension to be recognized				   +
+		Test with:															   +
+			wc -l inputFile.txt												   +
+			wc -l < Inputfile.txt											   +
+			wc -l < Inputfile.txt > outputFile								   +
+			w | wc -l 														   +
+			ls /usr/bin/ | head -10 | tail -5								   +
+			ls -ltr /usr/bin/ | grep grep | head -20 | tail -10				   + 	
+																			   +
+																			   +
\******************************************************************************/
...
void handle_parse(string input) {
	...
	string element_arr[send.size()];

	stringstream ssin(send);
	while(ssin.good()) {
		string temp;
		ssin >> temp;
		if (redir == true) {
